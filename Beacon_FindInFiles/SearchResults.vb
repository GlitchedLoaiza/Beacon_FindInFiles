                    ...
                    While True
                        ...
                        If line Is Nothing Then
                            For Each item In pending
                                item.Context = MatchContext.WithFocus(history, item.LineNumber)
                            Next
                            Exit While
                        End If
                        lineNumber += 1
                        Dim detail = If(stoppedMatching, Nothing, SearchDetail.FromRecord(query, line, $"Line {lineNumber:N0}", lineNumber, token:=token))
                        Dim captured = If(detail IsNot Nothing AndAlso detail.Context.Count > 0,
                                          detail.Context(0).Copy(), MatchContext.CaptureLine(line, lineNumber))
                        history.Enqueue(captured)
                        If history.Count > MatchContext.LineLimit Then history.Dequeue()
                        For Each item In pending.ToArray()
                            Dim following = captured.Copy()
                            following.IsMatchLine = False
                            item.Context.Add(following)
                            If item.Context.Count >= MatchContext.LineLimit Then pending.Remove(item)
                        Next
                        If detail IsNot Nothing Then
                            detail.Context = MatchContext.WithFocus(history.TakeLast(3), lineNumber)
                            result.Details.Add(detail)
                            pending.Add(detail)
