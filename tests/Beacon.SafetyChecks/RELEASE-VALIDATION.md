...
**Beacon Theme follow-up:** the current source includes a fourth, system-aware theme with muted-crimson/off-white buttons and coordinated red focus/selection accents. It is not in the historical locally published artifacts recorded here. Their checksums are preserved as historical evidence only. A theme-enabled release requires a fresh publish, hashes and release smoke checks; source promotion does not upload or replace an existing release.

**Current source review:** see [source promotion validation](../../docs/releases/2.1.0/PREPARATION.md#source-promotion-validation) for the current build/regression results and local-only master promotion scope. The same [preparation record](../../docs/releases/2.1.0/PREPARATION.md) retains the archived pre-theme 2.1.0 EXE/ZIP hashes, included notices, and 71-group Release run. That historical package used the Folder profile with ReadyToRun disabled and was unsigned. Neither its hashes nor the older smoke results below verify a theme-enabled build.
