...
**Current 2.2.0 release:** see [Beta → Release promotion](../../docs/releases/2.2.0/PROMOTION.md) for authorized reconciliation, final source/package provenance, validation and upload handoff. The earlier [candidate preparation](../../docs/releases/2.2.0/PREPARATION.md) and the records below remain historical evidence; their hashes must not be used to verify a different executable. Source promotion does not itself upload GitHub Release assets or sign the application.

**Historical Beacon Theme follow-up:** the theme-enabled 2.1.0 package is recorded separately under [packages/7dbb342](../../docs/releases/2.1.0/packages/7dbb342/release-manifest.json). The older local candidate checksums and smoke results on this page retain their original scope and must not be reused for a different build.

**Earlier 2.1.0 source review:** see [source promotion validation](../../docs/releases/2.1.0/PREPARATION.md#source-promotion-validation) for the build/regression results and local master promotion recorded at that time. The same [preparation record](../../docs/releases/2.1.0/PREPARATION.md) retains both earlier candidate and subsequent package evidence. Those historical packages were unsigned and used ReadyToRun-off packaging; preserve their notices and provenance rather than relabeling them as 2.2.0 validation.
...
**Historical Step 12 result: automated validation and a limited published-executable smoke test passed for the baseline below. Distribution approval remains subject to the manual gates.** This is not a claim that every archive, Windows installation, or provider configuration is supported without further testing. No release was uploaded, app version changed, or signing operation performed during that validation pass.
...
The regression runner is a console executable, not a Test Explorer test assembly. At this historical snapshot, its 69 groups included many assertions covering real archive fixtures, temporary pipelines, WPF templates, WebView2 rendering and headless service behavior. Current source-validation results are recorded in the preparation record linked above. Automated checks do not substitute for the following checks on the exact shipping executable.
...
- [ ] **Windows UX:** Check Light/Dark/System and Beacon Theme with Windows light and dark app preferences. Verify readable text and hover/pressed/disabled/selected states, native caption buttons, snapping, dragging, maximized/minimized restoration, high contrast, keyboard navigation, Help and regex link, date pickers, side panes and popup placement at different DPI scales and across monitors. Reduced-motion behavior must remain usable.
...
- [ ] **Target-machine runtime:** Test on intended Windows x64 machines, including one without the .NET SDK. Test with WebView2 present and absent and under relevant endpoint-security/AppLocker restrictions. A self-contained .NET executable does not bundle the WebView2 browser runtime. Validate additional Windows versions before claiming support, and check user-writable temporary-directory requirements.
...
- [ ] **Version/update contract:** Verify the intended release version consistently in Version, AssemblyVersion and FileVersion. Current source uses 2.1.0 / 2.1.0.0; the historical candidate above remains 2.0.1. Confirm the GitHub release tag/title is parseable by the checker. Version changes require rebuilding and regenerating evidence/hash.
...
- [ ] **Documentation and release assets:** Have a first-time user follow Help, review known limitations, choose the correct publish configuration, and prepare release notes and exact download checksum. Review repository license/ownership requirements with the release owner. This historical validation record does not itself authorize branch promotion or publication; the preparation record defines the separate local source-promotion scope.
