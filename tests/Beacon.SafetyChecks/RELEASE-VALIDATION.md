...
**Current 2.2.0 preparation:** see [Beta → Release preparation](../../docs/releases/2.2.0/PREPARATION.md) for the version-bumped candidate, fresh validation, package checksums, and promotion gates. The records below describe earlier builds and remain historical evidence; they must not be used to verify the 2.2.0 executable. Current preparation does not perform a merge, push, signing operation, or release upload.
...
**Earlier 2.1.0 preparation:** see [Beacon 2.1.0 preparation](../../docs/releases/2.1.0/PREPARATION.md) for that preparation's local artifacts, exact EXE/ZIP hashes, included notices, 71 passing Release regression groups, and later-promotion checklist. That package uses the existing Folder profile with ReadyToRun disabled and remains unsigned. The historical candidate hashes and smoke results below must not be reused as verification for a different build.
